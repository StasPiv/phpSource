...
class PhpFile extends PhpElement
{
    ...
    /**
     *
     * @var array Array of strings, the strings are use class names
     * @access private
     */
    ...
    public function getSource()
    {
        ...
        if (count($this->useClassNames) > 0) {
            foreach($this->useClassNames as $key => $value) {
                $ret .= 'use ' . $value . (is_numeric($key) ? '' : (' as ' . $key)) . ';' . PHP_EOL;
            }
            $ret .= PHP_EOL;
        }
     ...
     *
     * @param array $useClassNames The useClassNames to set
     */
    ...
    /**
     * Checks if the file has a use class names
     *
     * @return bool Returns true if a useClassNames is added to the file
     */
    public function hasUseClassNames()
    {
        return (count($this->useClassNames) > 0);
    }
